...
public class EssentialsTreeCommand extends EssentialsCommand {
    ...
    @Override
    protected final void run(final MinecraftServer server, final User user, final String commandLabel, final String[] args) throws Exception {
        run(server, user.getSource(), commandLabel, args);
    }

    @Override
    protected final void run(final MinecraftServer server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception {
        if (args.length == 0) {
            if (sender.isPlayer() && sender.getUser() != null) {
                runDefault(sender.getUser(), commandLabel);
            } else {
            ...
            if (node != null) {
                ...
                if (sender.isPlayer() && sender.getUser() != null) {
                    node.run(sender.getUser(), commandLabel, newArgs);
                } else {
    ...
    @Override
    protected final List<String> getTabCompleteOptions(final MinecraftServer server, final User user, final String commandLabel, final String[] args) {
        return getTabCompleteOptions(server, user.getSource(), commandLabel, args);
    }

    @Override
    protected final List<String> getTabCompleteOptions(final MinecraftServer server, final CommandSource sender, final String commandLabel, final String[] args) {
        if (args.length == 1) {
            return publicNodes;
        }
        final EssentialsTreeNode node = nodes.get(args[0]);
        if (node != null) {
            final String[] newArgs = Arrays.copyOfRange(args, 1, args.length);
            if (sender.isPlayer() && sender.getUser() != null) {
                return node.tabComplete(sender.getUser(), commandLabel, newArgs);
            } else {
