...
/**
 * Grouped online-player listing shared by {@code /list} and the PLAYERLIST keyword.
 */
public final class PlayerList {
    private PlayerList() {
    }

    public static String listUsers(final Essentials ess, final List<User> users, final String seperator) {
        ...
        for (final User user : users) {
            ...
            user.setDisplayNick();
            groupString.append(Text.get().legacyToMini(user.getDisplayName()));
    ...
    public static String listSummary(final Essentials ess, final User user, final boolean showHidden) {
        int playerHidden = 0;
        int hiddenCount = 0;
        ...
        final int max = ess.getServer().getMaxPlayers();
        if (hiddenCount > 0) {
            tlKey = "listAmountHidden";
            objects = new Object[] {ess.getOnlinePlayers().size() - playerHidden, hiddenCount, max};
        } else {
            tlKey = "listAmount";
            objects = new Object[] {ess.getOnlinePlayers().size() - playerHidden, max};
        }
        return user == null ? tlLiteral(tlKey, objects) : user.playerTl(tlKey, objects);
    }

    public static Map<String, List<User>> getPlayerLists(final Essentials ess, final User sender, final boolean showHidden) {
    ...
    public static List<User> getMergedList(final Essentials ess, final Map<String, List<User>> playerList, final String groupName) {
    ...
    public static String listGroupUsers(final Essentials ess, final Map<String, List<User>> playerList, final String groupName) throws Exception {
        ...
        final String displayGroupName = Character.toTitleCase(groupName.charAt(0)) + groupName.substring(1);
    ...
    public static String outputFormat(final String group, final String message) {
        return tlLiteral("listGroupTag", FormatUtil.replaceFormat(group)) + message;
    }

    public static List<String> prepareGroupedList(final Essentials ess, final CommandSource source, final String commandLabel, final Map<String, List<User>> playerList) {
        ...
        for (final String onlineGroup : onlineGroups) {
            ...
            if (!ess.getPermissionsHandler().hasLuckPerms() && asterisk.isEmpty() && groupName.equals("default")) {
